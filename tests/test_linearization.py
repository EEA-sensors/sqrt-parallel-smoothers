...
# @pytest.mark.parametrize("sqrt", [True, False])
# def test_linear_1(dim_x, dim_q, seed, method, sqrt):
#     # TODO: use get_system to reduce the number of lines
#     np.random.seed(seed)
#     a = np.random.randn(dim_x, dim_x)
#     b = np.eye(dim_x, dim_q)
#     c = np.random.randn(dim_x)
#
#     m_x = np.random.randn(dim_x)
#     m_q = np.random.randn(dim_q)
...
# def test_weights_points(dim_x, ):
#     print("sum_wm = ", np.sum(weights(dim_x)[0]))
#     print("sum_wc = ", np.sum(weights(dim_x)[1]))
#     print("xi = ",weights(dim_x)[2])
#
#     mean = np.random.randn(dim_x)
#     chol = np.random.randn(dim_x, dim_x)
#
#     wm = weights(dim_x)[0]
#     xi = weights(dim_x)[2]
#
#     points = jnp.repeat(mean.reshape(1, -1), wm.shape[0], axis=0) + jnp.dot(chol, xi.T).T
#     print("sigma_points = ", points)
#     print("mean = ", jnp.repeat(mean.reshape(1, -1), wm.shape[0], axis=0))
#     print("chol = ", chol.shape)
#     print(jnp.dot(chol, xi.T).T)
#     print(jnp.dot(chol, xi.T[:,0]))
