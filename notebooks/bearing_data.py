...
def _transition_function(x, dt, dtype=jnp.float_):
...
def _observation_function(x, s1, s2, dtype=jnp.float_):
    """
    ...
    return jnp.array([jnp.arctan2(x[1] - s1[1], x[0] - s1[0]),
                      jnp.arctan2(x[1] - s2[1], x[0] - s2[0])], dtype=dtype)
...
def make_parameters(qc, qw, r, dt, s1, s2, dtype=jnp.float_):
...
def _get_data(x, dt, a_s, s1, s2, r, normals, observations, true_states, dtype):
    for i, a in enumerate(a_s):
        ...
        observations[i] = [y1, y2]
        true_states[i] = np.concatenate((x, np.array([a], dtype=dtype)))
    return true_states, observations


def get_data(x0, dt, r, T, s1, s2, q=10., random_state=None, dtype=np.float32):
    """
    Parameters
    ----------
    x0: array_like
    ...
    random_state: np.random.RandomState or int, optional
        numpy random state
    dtype: optional
        dtype of the result, default is float32
    Returns
    -------
    ts: array_like
        array of time steps
    ...
    a_s = 1 + q * dt * np.cumsum(random_state.normal(0, 1, T).astype(dtype))
    s1 = np.asarray(s1, dtype=dtype)
    s2 = np.asarray(s2, dtype=dtype)

    x = np.copy(x0).astype(dtype)
    observations = np.empty((T, 2), dtype=dtype)
    true_states = np.zeros((T + 1, 5), dtype=dtype)
    ts = np.linspace(dt, (T + 1) * dt, T, dtype=dtype)
    true_states[0, :4] = x
    normals = random_state.randn(T, 2).astype(dtype)

    _get_data(x, dt, a_s, s1, s2, r, normals, observations, true_states[1:], dtype)
