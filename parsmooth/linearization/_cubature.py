...
def _get_sigma_points(mvn: MVNSqrt) -> Tuple[SigmaPoints, jnp.ndarray]:
    ...
    wm, wc, xi = _cubature_weights(n_dim, dtype)
...
def _cubature_weights(n_dim: int, dtype) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    xi: np.ndarray
        Orthogonal vectors
    """
    wm = np.ones(shape=(2 * n_dim,), dtype=dtype) / (2 * n_dim)
    wc = wm
    eye = np.eye(n_dim, dtype=dtype)
    xi = np.concatenate([eye, -eye], axis=0) * np.sqrt(n_dim)
